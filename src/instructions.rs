...
impl Instr {
    ...
    pub const fn length(&self) -> usize {
        1 + match self {
            Self::Ld8(dst, src) | Self::LdInc(dst, src) | Self::LdDec(dst, src) => dst.imm_bytes() + src.imm_bytes(),
            Self::Ld16(dst, src) => dst.imm_bytes() + src.imm_bytes(),
            Self::LdHlSpOffset | Self::AddSp | Self::Jr(_) | Self::Cb => 1,
            Self::Jp(_) | Self::Call(_) => 2,
            Self::Add(src) | Self::Adc(src) | Self::Sub(src) | Self::Sbc(src) | Self::And(src) | Self::Xor(src) | Self::Or(src) | Self::Cp(src) => {
                src.imm_bytes()
            }
            _ => 0,
        }
